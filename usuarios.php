...
// itemS
//=============================================================================
$sql  = "select nome, id_usuario  ";
$sql .= " from usuario ";
// $sql .= " where id_usuario in( 5, 11 ) ";
$sql .= " order by 1";
...
try {
    ...
    foreach( $rows as $r)
    {
      ...
      logar( $fileLog, $r['nome'] );

      $item  = $prefixo . '{';
      $item .= '"title":"' . $r['nome'] . ' - ' . $r['id_usuario'] . '"';
      $item .= ',"id":"' . $r['id_usuario'] . '"';
      $item .= ',"id_item":"' . $r['id_usuario'] . '"';
      $item .= ',"class":"success"';
      $item .= ',item: ' . getOportunidades( $r['id_usuario'] );
      $item .= '}';
      $prefixo = ',';
      $items[] = $item;
...
function getOportunidades( $idUsuarioP )
{

    global $conn, $fileLog, $mensagem;

    $sql = "select titulo ||' ' as nome, id_oportunidade
      , descricao
      , email, fone
...
boards: [
 {"title":"TRIAGEM","id_item":"1","id":"TRIAGEM"} 
 ,{"title":"ENTENDER A NECESSIDADE DO CLIENTE","id_item":"2","id":"ENTENDERANECESSIDADEDOCLIENTE"} 
 ,{"title":"TOP SERVICE DE PRODUTO E CONCEITO","id_item":"3","id":"TOPSERVICEDEPRODUTOECONCEITO"} 
 ,{"title":"PROJETO E PLANTA BAIXA","id_item":"4","id":"PROJETOEPLANTABAIXA"} 
 ,{"title":"CONTATO COM O PRODUTO","id_item":"5","id":"CONTATOCOMOPRODUTO"} 
 ,{"title":"ORCAMENTO","id_item":"6","id":"ORCAMENTO"} 
 ,{"title":"NEGOCIACAO","id_item":"7","id":"NEGOCIACAO"} 
 ,{"title":"FECHAMENTO","id_item":"8","id":"FECHAMENTO"} 
 ]
        boards: [
          {
            id: "_todo",
...
try {
  ...
  $rows = $result->fetchAll();
	...
	foreach( $rows as $r)
	{
        ...
        $iditem = ( $r['id_item'] == null)?0:$r['id_item'];  
        $linhas = ( $r['linhas'] == null)?0:$r['linhas'];  
        

		$item = '{"id":"'. $iditem . '","nome":"'. $nome . '","cor":"'.$cor.'","linhas":"'. $linhas . '"}';
...
<head>
    ...
    <link rel="stylesheet" href="./kanban/dist/jkanban.min.css" />
    <link
      href="https://fonts.googleapis.com/css?family=Lato"
      rel="stylesheet"
    />

    <link rel="stylesheet" href="./kanban/index.css" />
...
<div class="modal" id="myModal">
  <div class="modal-dialog">
    ...
    <script src="./kanban/dist/jkanban.js"></script>
