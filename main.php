...
$sql = "select nome, id_etapa 
, replace( nome, ' ', '') as codigo
, replace( e.bs_cor, 'bg-', '' ) as classe
from crm_etapa e
where fl_ativo = 'S'
order by id_etapa_2";
...
try {
    ...
    foreach( $rows as $r)
    {
      ...
      $etapa .= '"title":"' . $r['nome'] . '"';
      $etapa .= ',"id":"' . $r['codigo'] . '"';
      $etapa .= ',"id_etapa":"' . $r['id_etapa'] . '"';
      $etapa .= ',"class":"' . $r['classe'] . '"';
      $etapa .= ',item: ' . getOportunidades( $idUsuarioP, $r['id_etapa'], $r['nome'] );
      $etapa .= '}';
      $prefixo = ',';
      $etapas[] = $etapa;
...
function getOportunidades( $idUsuarioP, $idEtapaP, $nomeP )
{
    global $conn, $fileLog, $mensagem;
    logar( $fileLog, $nomeP );

    $sql = "select nome ||'<br><b>'|| to_char( o.dt_inclusao, 'dd/mm/yyyy' )||'&nbsp;'||cast( (current_date - dt_inclusao ) as varchar)||' dias</b>'  as nome
      , id_oportunidade
      , replace( replace( descricao, '<',''), '>','') as descricao
      , email, fone
       from crm_oportunidade o
       where id_usuario = $idUsuarioP 
         and id_etapa = $idEtapaP 
           and o.dt_inclusao is not null
    ...
    try {
        ...
        {
            $saida .= $prefixo . '{';
            $saida .= 'id:"'. $r['id_oportunidade']. '"';
            $saida .= ',title:"'. $r['nome']  . '"';
            
            $saida .= ', click: function (el ) {  ';               
            $saida .= "    exibir( '" . $r['id_oportunidade'] . "' );";
            $saida .= "  }";


            $saida .= ', drop: function(el) { ';
            $saida .= '      exibir("DROPPED: " + el.dataset.eid); ';
            $saida .= '   }';
...
<script>
      ...
      var KanbanTest = new jKanban({
        ...
        dropEl: function(el, target, source, sibling){
          KanbanTest.boardSelected = target.parentElement.getAttribute('data-id');
          KanbanTest.boardId = target.parentElement.getAttribute('data-id_etapa');
...
function exibir( id )
{
	...
	// app.exibir( p );
  console.log( id );
  console.log( KanbanTest.boardSelected );
  console.log( "Id Etapa: " + KanbanTest.boardId );
	$('#myModal').modal('show'); 
